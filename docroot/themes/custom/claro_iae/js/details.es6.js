...
(($, Modernizr, Drupal) => {
  ...
  Drupal.behaviors.claro_iaeDetails = {
    attach(context) {
      ...
      $(once('claro_iaeDetails', context === document ? 'html' : context)).on(
        'click',
        (event) => {
  ...
  Drupal.behaviors.claro_iaeDetailsToggleShim = {
    attach(context) {
      ...
      $(once('claro_iaeDetailsToggleShim', 'details .details-title', context)).on(
        'keypress',
        (event) => {
  ...
  Drupal.theme.detailsSummarizedContentWrapper = () =>
    `<span class="claro_iae-details__summary-summary"></span>`;
